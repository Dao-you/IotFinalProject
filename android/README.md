...
- 遊戲介面：
  ...
  - 可在簽到記錄底部刪除簽到記錄；刪除後會停止發射手機 beacon。
- 管理介面：
  - 新增檢核點名稱與 `Beacon Data`，預設會帶入下一組測試用 beacon data。
  - 選取檢核點後，可在地圖上放置或拖曳圖釘。
  - 選取檢核點後，可修改該檢核點名稱與 `Beacon Data`。
  - 可刪除選取的檢核點；刪除時會移除相關簽到記錄，若該檢核點正在作為手機 beacon 來源也會同步更新。
